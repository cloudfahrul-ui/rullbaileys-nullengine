...
const FALLBACK_TMP_DIR = process.env.BAILEYS_TMP_DIR || process.env.TMPDIR || "/home/container/tmp";
let cachedTmpDir;
...
const getTmpFilesDirectory = async () => ensureTempDirectory();
//=======================================================//
...
export const getRawMediaUploadData = async (media, mediaType, logger) => {
  ...
  const filePath = join(await getTmpFilesDirectory(), mediaType + generateMessageIDV2());
...
export async function generateThumbnail(file, mediaType, options) {
  ...
  if (mediaType === "image") {
    ...
    const imgFilename = join(await getTmpFilesDirectory(), generateMessageIDV2() + ".jpg");
    try {
...
export const encryptedStream = async (media, mediaType, { logger, saveOriginalFileIfRequired, opts } = {}) => {
  ...
  const encFilePath = join(await getTmpFilesDirectory(), mediaType + generateMessageIDV2() + "-enc");
  const encFileWriteStream = createWriteStream(encFilePath);
  let originalFileStream;
  let originalFilePath;
  if (saveOriginalFileIfRequired) {
    originalFilePath = join(await getTmpFilesDirectory(), mediaType + generateMessageIDV2() + "-original");
...
export const getWAUploadToServer = ({ customUploadHosts, fetchAgent, logger, options }, refreshMediaConn) => {
  return async (filePath, { mediaType, fileEncSha256B64, timeoutMs }) => {
    ...
    const buildHosts = (info) => [...(customUploadHosts || []), ...(info?.hosts || [])];

    let hosts = buildHosts(uploadInfo);
    ...
    for (let i = 0; i < hosts.length; i++) {
      ...
      try {
        ...
        logger.debug({
          ...
          parsedResult: result
        }, "upload host response");

        const mediaUrl = result?.url;
        const directPath = result?.direct_path || result?.directPath;
        ...
        if (mediaUrl || directPath) {
          ...
          logger.debug({ hostname, urls }, "media upload success");
          break;
        } else {
          uploadInfo = await refreshMediaConn(true);
          hosts = buildHosts(uploadInfo);

          throw new Error(
            `upload failed, status=${responseStatus}, body=${responseText?.slice(0, 500)}`
          );
        }
      } catch (error) {
